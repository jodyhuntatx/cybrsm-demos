...
main() {
  ...
  baseBranch=master
  mergeBranch=$(cat ${commitFile} | jq -r .safeRequest.projectName)
  commitPath="${mergeBranch}"
...
mergePullRequest() {
  ...
  case ${http_code} in
    201 | 204)
	echo "Merge successful." | tee >> $LOGFILE
	;;
    404)
	echo "Merge branch or base does not exist." | tee >> $LOGFILE
	;;
    409)
	echo "Merge conflict, please resolve." | tee $LOGFILE
	;;
  esac
}
