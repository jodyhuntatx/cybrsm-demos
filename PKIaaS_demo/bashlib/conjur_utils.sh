...
function conjur_authenticate {
	$util_defaults
    api_key=$(curl $CONJUR_VERBOSE --fail -s -k --user "$CONJUR_AUTHN_LOGIN:$CONJUR_AUTHN_API_KEY" $CONJUR_APPLIANCE_URL/authn/$CONJUR_ACCOUNT/login)
	urlLogin=$(echo "$CONJUR_AUTHN_LOGIN" | sed 's/\//%2F/g')
	session_token=$(curl $CONJUR_VERBOSE --fail -s -k --data "$api_key" $CONJUR_APPLIANCE_URL/authn/$CONJUR_ACCOUNT/$urlLogin/authenticate)
