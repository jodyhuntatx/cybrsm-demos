...
ConfigureSSHDConfig()
{
	...
	WriteLog "Configuring the [sshd_config] file..."
	
	SetSSHDConfigProperty "PermitEmptyPasswords" "yes" $SSHD_CONFIG_F
	SetSSHDConfigProperty "UseDNS" "no" $SSHD_CONFIG_F
	
	selinuxEnforcing=$(sestatus | grep enabled )
	
	# If we found that SELinux is enforcing, set neccessary sshd parameters to work.
	if [ ! -z "$selinuxEnforcing" ] ; then
		SetSSHDConfigProperty "UsePAM" "yes" $SSHD_CONFIG_F
		SetSSHDConfigProperty "ChallengeResponseAuthentication" "no" $SSHD_CONFIG_F
	fi
	
	#restore backed up PSMP_AdditionalDelimiter value
	if [ ! -z "$PSMP_AddDelimValueBackup" ] ; then
		AddNewSSHDConfigProperty "$PSMP_AddDelimValueBackup" $SSHD_CONFIG_F
	fi
	#restore backed up PSMP_OpenSSHLogFolder value
	if [ ! -z "$PSMP_OpenSSHLogFValueBackup" ] ; then
		AddNewSSHDConfigProperty "$PSMP_OpenSSHLogFValueBackup" $SSHD_CONFIG_F
	fi
	#restore backed up PSMP_OpenSSHTraceLevels value
	if [ ! -z "$PSMP_OpenSSHTrcLevValueBackup" ] ; then
		AddNewSSHDConfigProperty "$PSMP_OpenSSHTrcLevValueBackup" $SSHD_CONFIG_F
	fi
	#restore backed up PSMP_TargetAddressPortAdditionalDelimiter value
	if [ ! -z "$PSMP_TargetAddrPortDelimValueBackup" ] ; then
		AddNewSSHDConfigProperty "$PSMP_TargetAddrPortDelimValueBackup" $SSHD_CONFIG_F
	fi
	#restore backed up PSMP_MaintenanceUsers value
	if [ ! -z "$PSMP_MaintUsersValueBackup" ] ; then
		AddNewSSHDConfigProperty "$PSMP_MaintUsersValueBackup" $SSHD_CONFIG_F
	fi
	WriteLog "Configuring [sshd_config] file has finished."
}


ConfigureSSHDServiceInitScript()
{
    #if SystemD is used, like in RHEL7 and we did not installed our OpneSSH, then Init scripts are not in used - /etc/init.d does not exists.
	if [ ! -f $SSHD_SRV_INITD_F ] ; then
     return
    fi

	WriteLog "Configuring the [sshd] service initialization script..."

	sed -i 's|.*[^#]/*/sbin/restorecon /etc/ssh/ssh_host_ecdsa_key.pub|#/sbin/restorecon /etc/ssh/ssh_host_ecdsa_key.pub|g' $SSHD_SRV_INITD_F
	TestForErrors "Failed to configure the [sshd] service initialization script."
	
	WriteLog "Configuring the [sshd] service initialization script has finished."
}

#For RHEL7 or above
SetSSHPrivateKeysPermissions()
{
	#Do not continue if RHEL5 or RHEL6
	if [ "$IsRHEL5" -gt "0" ] || [ "$IsRHEL6" -gt "0" ] ; then
		return
	fi
...
ReplaceSSHDExecutable()
{
	...
	if [ "$?" == "$RC_SUCCESS" ] ; then
		...
		$SSHD_SRV_CMD restart >/dev/null 2>&1
...
UpdateSSHD()
{
	...
	#configure sshd's initialization script
	ConfigureSSHDServiceInitScript
