...
main() {
  ...
  if [[ $command != token_sa ]]; then
    getOauthToken $IDENTITY_ADMIN_USER $IDENTITY_ADMIN_PWD
  fi
  ...
  case $command in
    ...
    keys)
        ...
        ;;
    token_sa)
        getAuthToken $CONJUR_ADMIN_USER $CONJUR_ADMIN_PWD
        exit
        ;;
    token)
	echo $AUTH_TOKEN
