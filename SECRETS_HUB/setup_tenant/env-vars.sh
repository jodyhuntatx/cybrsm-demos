...
if $SELF_HOSTED_PAM; then
  ...
  # domain for user identities
  export TENANT_DOMAIN="cyberark.cloud.13576"
  ...
  PCLOUD_TENANT_URL=https://sh-immersion.cyberark.cloud
fi
...
if $SELF_HOSTED_PAM; then
  ...
  export VAULT_BASE_URL=https://$CYBERARK_SUBDOMAIN_NAME.privilegecloud.cyberark.cloud/PasswordVault
  export VAULT_API_URL=$VAULT_BASE_URL/api
